...
async function fire(ctx, signed) {
  ...
  const sent = await provider.broadcastTransaction(signed.raw);
  ...
  while (totalWon < countTarget && totalAttempts < attemptCap && active.length) {
    ...
    for (const wallet of active) {
      for (let shot = 0; shot < burst; shot++) {
        if (totalAttempts + batch.length >= maxAttempts) break;
...
async function main() {
  ...
  const ctx = { contract, provider, wallets, opts: { gasLimitOverride: args.gasLimitOverride, feeBumpPct: args.feeBumpPct } };
